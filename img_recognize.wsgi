...
def application(env, start_response):
    ...
    queue_dict['image_uri'] = "https://tishka.org/master%20class/chicken%20Kiev/main.jpg"
    ...
    try:
        with open(current_filename, "r", encoding="utf-8") as f:
            ...
            if (queue_dict["status"] == "Ok"):
                ...
                #full_data, label_stats, source_labels
                max_dish = {}
                max_dish["dish_weight"] = -1
                n = 0
                for dish in full_data:
                    n += 1
                    #out_s += "<p>" + str(dish["title"]) +  ": " + str(n)
                    ...
                    for item in label_annotations:
                        #out_s += "<li>" + item + ": " + str(label_stats[item])
                        if (item in source_labels):
                            if (label_stats[item] > 0):
                                #dish_weight = dish_weight * label_stats[item]
                                this_weight = label_stats[item]
                                dish_weight = dish_weight * this_weight / (dish_weight*this_weight + (1-dish_weight)*(1-this_weight))
                                
                                #out_s += "; dish_weight: " + str(dish_weight)
                    #out_s += "</ul>"
                    #out_s += "<b>dish_weight</b>: " + str(dish_weight) + "<br>"

                    #out_s += "<li>" + dish["title"] +": "
                    if (dish_weight == 1):
                        dish_weight = 0

                    if (dish_weight > max_dish["dish_weight"]):
                        max_dish["dish_weight"] = dish_weight
                        max_dish["dish"] = dish
                        max_dish["label_annotations"] = label_annotations
                        max_dish["n"] = n
                    
                    
                out_s += "<b>" + str(max_dish["dish"]["title"]) + ": </b>" + str(max_dish["dish_weight"]) + "<br>"
                out_s += "<ul>"
                dish_weight = 0.5
                for max_label in max_dish["label_annotations"]:
                    out_s += "<li>" + max_label + ": " + str(label_stats[max_label])
                    if (max_label in source_labels):
                        if (label_stats[max_label] > 0):
                            this_weight = label_stats[item]
                            dish_weight = dish_weight * this_weight / (dish_weight*this_weight + (1-dish_weight)*(1-this_weight))

                        out_s += "; <b>" + str(dish_weight) + "</b>"
                out_s += "</ul>"
